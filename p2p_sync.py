...
def master_process(
    ...
    if graph_topology != "complete":
        # init a graph object corresponding to the right topology
        if graph_topology == "cycle":
            ...
            raise ValueError("Supported graph topologies are ['complete', 'cycle', 'exponential'].")
        ...
        if graph_topology == "complete":
            ...
            for j in N_i:
                ...
                if G.nodes[j]["is_ready_2_com"]:
                    ...
                    j_ready = j
                    # stop searching for an available neighbor
                    break
            # if the neighbors of i for next step is not ready
            if j_ready is None:
                # we signal that i is ready in the graph stored in memory
