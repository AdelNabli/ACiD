...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs='+',
        help="The values of (eta, beta_tilde) in the ACiD algorithm.",
    )
    parser.add_argument(
        "--batch_size",
        default=256,
...
def run(rank, local_rank, world_size, n_nodes, master_addr, master_port, args):
    ...
    com_message = '\n WORKER {}: Number of grad steps : {} , Number of comm : {}'.format(rank, count_grads, count_com)
    print(com_message)
    # put the info in the filestore in order to make it available to worker 0
    filestore.set("Worker {} #grad / #comm".format(rank), "{} / {}".format(count_grads, count_com))

    # if we are the worker 0, print/saves the logs
    if rank == 0:
        # wait all the workers
        filestore.wait(["Worker {} #grad / #comm".format(rank) for rank in range(world_size)])
        print("Final model: ")
        test_loader, _ = data_loader(rank, world_size, train=False, batch_size=args.batch_size, dataset_name=args.dataset_name)
        loss, correct, len_data, percent = evaluate(adp_model, test_loader, criterion, rank=0, print_message=True, dataset_name=args.dataset_name)
        grad_message = '\n Total training time for {} GPUS : {} minutes, {:.1f} secondes'.format(world_size, int(total_time//60), total_time%60)
        print(grad_message)
        # save results
        dict_result = create_dict_result(args, filestore, world_size, n_nodes, torch.cuda.get_device_name(), total_time, correct, len_data, percent, id_run)
