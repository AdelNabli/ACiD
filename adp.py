...
class ADP(nn.Module):
    ...
    def __init__(self, model, rank, local_rank, world_size, nb_grad_tot_goal, log, rate_com, apply_acid, acid_params, criterion, optimizer, data_iterator, momentum, dataset_name, graph_topology, deterministic_com):
        ...
        self.new_grads = mp.Value("i", 0) # nb of new grads between two comms to the master process
    ...
    def init_acid(self, acid_params, criterion, optimizer, data_iterator, momentum, dataset_name):
        ...
        if self.apply_acid:
            ...
            self.mom_vec = init_momentum_var(self.module, criterion, optimizer, data_iterator, self.local_rank, self.delta_t_grad, momentum, dataset_name)
    ...
    def step(self, optimizer, scheduler, normalize_grads):
        ...
        if self.apply_acid:
            ...
            acid_ode(self.params_com, self.params_com_tilde, self.ode_matrix, t_old, t_new, self.delta_t_grad.value)
        ...
        if self.apply_acid:
            ...
            self.delta_t_grad.value = 0.5 * delta_t_grad + (1 - 0.5) * self.delta_t_grad.value
        ...
        if self.rate_com < 1:
            if self.count_grads_local.value >= self.count_grads_next_wait:
                ...
                try:
                    ...
                    pass
                self.count_grads_next_wait += np.random.poisson(lam=1/self.rate_com, size=None)
        else:
            try:
                self.barrier_com_grad.wait()
            except:
